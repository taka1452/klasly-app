...
export default function OverageManagementPage() {
  ...
  function statusBadge(status: string) {
    ...
    return (
      <span className={`inline-block rounded px-2 py-0.5 text-xs font-medium ${colors[status] || "bg-gray-100 text-gray-500"}`}>
    ...
    <div>
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-gray-500">
          View and manage instructor overage charges by month.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <>
                ...
                <tbody className="divide-y">
                  {charges.map((c) => (
                    <tr key={c.id} className="hover:bg-gray-50">
                      <td className="px-4 py-3 font-medium text-gray-900">{c.instructor_name}</td>
                      ...
                      <td className="px-4 py-3 text-red-600 font-medium">{fmtMinutes(c.overage_minutes)}</td>
          ...
          <div className="mt-4 text-right text-sm text-gray-700">
            Total overage charges: <span className="font-semibold">${(totalCharge / 100).toFixed(2)}</span>
            {charges.length > 0 && (
          ...
          {waivingId && (
            <div className="card mt-4 border-amber-200 bg-amber-50">
              <h3 className="text-sm font-semibold text-gray-900">Waive Overage Charge</h3>
              ...
              <div className="mt-3 flex gap-2">
                ...
                <button
                  onClick={() => { setWaivingId(null); setWaiveReason(""); }}
